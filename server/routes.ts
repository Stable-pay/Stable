...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Token withdrawal endpoints
  app.post('/api/withdrawal/token', async (req, res) => {
    try {
      const { userAddress, tokenSymbol, tokenAmount, inrAmount, transactionHash } = req.body;
      ...
      const transaction = await storage.createTransaction({
        ...
        metadata: JSON.stringify({
          ...
          withdrawalType: 'crypto_to_inr'
        })
      });

      console.log(`Token withdrawal: ${tokenAmount} ${tokenSymbol} = ₹${inrAmount} for ${userAddress}`);

      res.json({
        success: true,
        transactionId: transaction.id,
        message: 'Token withdrawal processed successfully'
      });
    } catch (error) {
      console.error('Token withdrawal failed:', error);
      res.status(500).json({ error: 'Token withdrawal failed' });
    }
  });

  // Get KYC status for user
  app.get('/api/user/kyc-status/:address', async (req, res) => {
    try {
      const { address } = req.params;
  ...
  app.get('/api/withdrawals/:address', async (req, res) => {
    try {
      ...
      const withdrawals = await storage.getTransactions(user.id);
      const tokenWithdrawals = withdrawals.filter(t => t.type === 'token_withdrawal');

      res.json(tokenWithdrawals.map(w => ({
        id: w.id,
        tokenSymbol: w.fromToken,
        tokenAmount: w.fromAmount,
        inrAmount: w.toAmount,
        status: w.status,
        transactionHash: w.txHash,
        timestamp: w.createdAt,
        metadata: w.metadata ? JSON.parse(w.metadata as string) : {}
      })));
    } catch (error) {
  ...
  // Token balance endpoint for supported tokens
  app.get('/api/tokens/balance/:address', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'Token balance fetch failed' });
    }
  });

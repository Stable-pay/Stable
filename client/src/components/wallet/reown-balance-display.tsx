/**
 * Reown AppKit Balance Display Component
 * Shows real-time token balances from connected wallets
 */

import { useReownWalletBalances, useNativeBalance } from '@/hooks/use-reown-wallet-balances'
...
export function ReownBalanceDisplay() {
        ...
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          <div>
            ...
            <p className="text-sm text-purple-600">
              {address?.slice(0, 6)}...{address?.slice(-4)} on {getNetworkName(currentChain)}
            </p>
          </div>
          <Button
            onClick={refreshBalances}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-purple-500">{token.chainName}</p>
                    </div>
                  </div>
        ...
        <Card className="bg-purple-50 border-purple-200">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-purple-600 text-sm mb-4">
              Your wallet appears to be empty on {getNetworkName(currentChain)}
            </p>
...
/**
 * Simplified balance display for token selection
 */
export function ReownTokenSelector({ onTokenSelect }: { onTokenSelect: (token: any) => void }) {
  const { balances, isLoading } = useReownWalletBalances()
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid gap-2">
        {balances.slice(0, 5).map((token, index) => (
          <Button
      ...
      {balances.length > 5 && (
        <p className="text-xs text-purple-500 text-center">
          Showing top 5 tokens. Total: {balances.length}
        </p>
      )}
    </div>
  )
...
function getNetworkName(chainId: number | null): string {
  ...
  return chainId ? networkNames[chainId] || `Chain ${chainId}` : 'Unknown Network'
}